...
function getStatus(p: PromoCode) {
  ...
  const now = new Date();
  if (p.startDate && now < new Date(p.startDate))
    return { label: "Programmé", color: "var(--gold)" };
  if (p.endDate && now > new Date(p.endDate))
    return { label: "Expiré", color: "#aaa" };
  if (p.maxUses && p.currentUses >= p.maxUses)
...
export default function PromotionsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Codes promo"
        subtitle="Réductions et promotions"
      ...
      {filtered.length === 0 ? (
        ...
        <ListTable
          columns="140px minmax(220px, 1fr) 100px 120px 80px 100px 190px"
          minWidth={980}
          header={
            <>
              <span>Code</span>
          ...
          {filtered.map((p) => {
            ...
            return (
              <ListRow
                key={p.id}
                onOpen={() => openEdit(p)}
                openLabel={`Modifier le code promo ${p.code}`}
              >
                <span
                  style={{
                    fontSize: 13,
                ...
                <div>
                  ...
                  {p.endDate && (
                    <div style={{ fontSize: 11, color: "var(--grey-3)" }}>
                      Expire : {new Date(p.endDate).toLocaleDateString("fr-FR")}
                    </div>
                  )}
                </div>
                <Badge color={status.color}>{status.label}</Badge>
                <Badge color={TYPE_MAP[p.discountType].color}>
                  {TYPE_MAP[p.discountType].label}
                </Badge>
                <span
                  style={{
                    fontSize: 13,
        ...
        <Modal onClose={() => setShowForm(false)}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <Field label="Nb max. d'utilisations">
                <input
                  type="number"
                  min={1}
                  style={inputStyle}
            ...
            <FormButtons
              ...
              submitLabel={editId ? "Enregistrer" : "Créer le code"}
            />
          </form>
        </Modal>
      )}
