...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <AdminLayout title="Dashboard">
        <div
          style={{ textAlign: "center", padding: 60, color: "var(--grey-3)" }}
        >
          Chargement...
        </div>
      </AdminLayout>
  ...
  const tauxConversion =
    devis.total > 0 ? Math.round((devisAcceptes / devis.total) * 100) : 0;
    ...
    <AdminLayout title="Dashboard">
      ...
      >
        ...
        {kpiCard(
          ...
          devis.total,
          "var(--blue)",
          `${devisEnAttente} en attente`,
        )}
        {kpiCard(
          "Factures",
          factures.total,
          "var(--gold)",
          `${facturesEnvoyees} en attente`,
        )}
        {kpiCard(
          "Encaissé",
          `${factures.totalPaid.toFixed(0)}€`,
          "var(--green)",
        )}
        {kpiCard(
          "En attente",
          `${facturesEnAttenteTotal.toFixed(0)}€`,
          "var(--gold)",
        )}
        {kpiCard(
          "Taux conversion",
          `${tauxConversion}%`,
          tauxConversion >= 50 ? "var(--green)" : "var(--gold)",
          `${devisAcceptes}/${devis.total} devis`,
        )}
      </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Object.entries(DEVIS_STATUS_LABELS).map(([key, label]) => {
              ...
              const pct =
                devis.total > 0 ? Math.round((count / devis.total) * 100) : 0;
              return (
                <div key={key}>
                  <div
                    style={{
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Object.entries(FACTURE_STATUS_LABELS).map(([key, label]) => {
              ...
              const pct =
                factures.total > 0
                  ? Math.round((count / factures.total) * 100)
                  : 0;
              return (
                <div key={key}>
                  <div
          ...
          >
            ...
            <span
              style={{ fontSize: 14, fontWeight: 700, color: "var(--green)" }}
            >
              {factures.totalPaid.toFixed(0)}€
            </span>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {devis.recent.map((d) => (
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 12, color: "var(--grey-3)", flex: 1 }}
                  >
                    {d.client?.company}
                  </span>
                  <span
                  ...
                  <span style={badge(STATUS_COLORS[d.status])}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {factures.recent.map((f) => (
                ...
                >
                  ...
                  <span style={badge(STATUS_COLORS[f.status])}>
